...
import quantum.tests.unit.l3.testlib_l3association as testlib
...
class L3AssociationAbstractAPITest(L3AbstractAPITest):
    ...
    def _create_association(self, format, subnet_id, routetable_id,
    ...
    def _test_create_association(self, format):
        """Test creating a new association"""
        LOG.debug("_test_create_association - format:%s - START", format)
        content_type = "application/%s" % format
        subnet_id = self._create_subnet_id(format)
        routetable_id = self._create_routetable_id(format)
        association_data = self._create_association(format, subnet_id,
                                                    routetable_id)
        ...
        LOG.debug("_test_create_association - format:%s - END", format)

    def _test_create_assoc_badreq(self, format):
        """Test for a bad request while creating association"""
        LOG.debug("_test_create_association_badrequest - format:%s - START",
                  format)
        bad_body = {'association': {'bad-attribute': 'very-bad'}}
        subnet_id = self._create_subnet_id(format)
        routetable_id = self._create_routetable_id(format)
        self._create_association(format, subnet_id, routetable_id,
                                 custom_req_body=bad_body,
                                 expected_res_status=400)
        self._create_association(format, "BAD", "BAD",
                                 custom_req_body=bad_body,
                                 expected_res_status=400)
        LOG.debug("_test_create_association_badrequest - format:%s - END",
                  format)

    def _test_show_association(self, format):
        """Test to show available association"""
        LOG.debug("_test_show_association - format:%s - START", format)
        content_type = "application/%s" % format
        subnet_id = self._create_subnet_id(format)
        routetable_id = self._create_routetable_id(format)
        create_association_data = self._create_association(format, subnet_id,
                                                           routetable_id)
        show_association_req = testlib.show_association_request(self.tenant_id,
                                                                subnet_id,
                                                                format)
        ...
        LOG.debug("_test_show_association - format:%s - END", format)

    def _test_show_assoc_not_found(self, format):
        """Test to check if no show association found"""
        LOG.debug("_test_show_association_not_found - format:%s - START",
                  format)
        ...
        LOG.debug("_test_show_association_not_found - format:%s - END", format)

    def _test_delete_association(self, format):
        """Test deleting association"""
        LOG.debug("_test_delete_association - format:%s - START", format)
        #content_type = "application/%s" % format
        subnet_id = self._create_subnet_id(format)
        routetable_id = self._create_routetable_id(format)
        self._create_association(format, subnet_id, routetable_id)
        ...
        LOG.debug("_test_delete_association - format:%s - END", format)

    def _create_subnet_id(self, format):
